  ...
  return (
    <div className="mx-auto flex min-h-dvh w-full max-w-6xl flex-col gap-4 p-4 md:p-6">
      <header className="flex items-center justify-between">
        ...
        <Button variant="secondary" className="rounded-2xl" onClick={() => (location.href = "/app/wiki")}>Voltar</Button>
      </header>
      ...
      <Card className="rounded-2xl">
        <CardHeader>
          <CardTitle className="text-base">Criando como: {activePersona?.name ?? "—"}</CardTitle>
        </CardHeader>
        ...
        <CardContent className="space-y-3">
          <div className="space-y-2">
            ...
            <input ref={coverInputRef} type="file" accept="image/*" className="hidden" onChange={(e) => {
              ...
              void uploadCover(file)
                ...
                .catch((err: unknown) => {
                  const message = err instanceof Error ? err.message : "Falha ao enviar capa.";
                  toast.error(message);
                });
            }} />

            <div className="flex gap-2">
              <Button type="button" variant="secondary" className="rounded-2xl" onClick={() => coverInputRef.current?.click()}>
                Enviar capa
              </Button>
          ...
          <Input placeholder="Título da wiki" value={title} onChange={(e) => setTitle(e.target.value)} />
